...
def callback(): # callback function
    ...
    if chvar.get()==1: # means the checkbox is checked
        ...
        print('not selected')
        # this is where to get our gender value to show we run the program
...
# Checkbox variale
cbox=Checkbutton(root,text='Remember Me', variable=chvar, font='Arial 16').grid(row=4, column=0, sticky=E, columnspan=2) # in order to ger it align right
...
ttk.Radiobutton(root, text='Female', var=gender).grid(row=5, column=0)
...
"""ComboBox = ttk.Combobox(root, textvariable=months, values=('Jan', 'Feb', 'Mar', 'Apr', 'May', 'July, 'Aug', 'Sep', 'Oct', 'Nov', 'Dec')).grid(row=6, column=0)"""

# There is a problem that when we run the program and click on a month we can actually write over it pr delete it

ComboBox = ttk.Combobox(root, textvariable=months, values=('Jan', 'Feb', 'Mar', 'Apr', 'May', 'July', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'), state='readonly').grid(row=6, column=0)

# Create spinbox - 'from' is a special keyword for Pythin - so we need to use _ after it
year = StringVar()
# Spinbox(root, from_=1990, to=2022, textvariable=year).grid(row=6, column=1)
# Information below:
# Use grid geometry Manager to out window but we are unable to edit the values in the spinbox
Spinbox (root, form_=1990, to=2022, textvariable=year, state='readonly').grid(row=6, column=1)
